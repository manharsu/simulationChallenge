...
bins = np.logspace(np.log10(min(final_balances)), np.log10(max(final_balances)), 30)
hist, bins, patches = ax.hist(final_balances, bins=bins, alpha=0.7, color='#2E86AB', edgecolor='black')

# Add reference lines
ax.axvline(x=initial_wealth, color='red', linestyle='--', linewidth=2, label=f'Initial Investment (${initial_wealth:,})')
ax.axvline(x=100000, color='orange', linestyle='--', alpha=0.7, label='$100,000 Reference')
ax.axvline(x=mean_balance, color='green', linestyle='-', alpha=0.8, label=f'Mean (${mean_balance:,.0f})')

# Formatting
ax.set_xscale('log')
ax.set_xlabel('Final Account Balance ($) - Log Scale', fontsize=12)
...
P(Balance > $30,000): {prob_above_initial:.1%}
P(Balance > $100,000): {prob_above_100k:.1%}
P(Bankrupt < $1,000): {prob_bankrupt:.1%}'''
...
years = 40
n_simulations = 100
bet_fraction = 0.5  # Bet exactly 50% of balance each round
...
# Comparative analysis with original strategy
original_final_balances = final_balances  # From previous section

# Create comparative visualization
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

# Plot 1: Side-by-side histograms
bins = np.logspace(0, 9, 40)  # Wider range to accommodate both strategies
...
ax1.set_title('Comparison: Final Balance Distributions', fontsize=14, fontweight='bold')
ax1.legend()
...
def simulate_median_path(strategy, n_years=years, n_sims=1000):
...
ax2.set_ylabel('Account Balance ($) - Log Scale', fontsize=12)
ax2.set_title('Median Wealth Paths Comparison', fontsize=14, fontweight='bold')
ax2.legend()
...
print("COMPARATIVE ANALYSIS: Original vs Modified Strategy")
print("=" * 50)
...
print(f"{'Maximum Balance':<25} ${np.max(original_final_balances):<13,.0f} ${np.max(modified_final_balances):<13,.0f}")
...
print(f"• Modified strategy has BETTER UPSIDE POTENTIAL: Higher probability of success ({modified_prob_above_30k:.1%} vs {np.mean(original_final_balances > 30000):.1%})")
print(f"• Modified strategy provides BETTER RISK-REWARD TRADEOFF: Higher median and more consistent outcomes")
...
The original strategy is **significantly riskier**, with a **63% probability** of near-total loss (**< $1,000**) compared to only **1%** for the modified strategy.

**Which has Better Upside?**

The modified strategy has **dramatically better upside characteristics**:

- **91% probability** of ending above **$30,000** vs only **22%** for original

- **Higher median outcome: $145,081** vs **$86**

- **More consistent wealth growth** without the **catastrophic drawdowns**

**Key Insight:**

By betting only **50% of the portfolio** each round, the modified strategy achieves the **optimal balance** between:

- **Participating in upside** during winning streaks

- **Preserving capital** during losing streaks

- **Avoiding ruin** through position sizing

The modified strategy transforms the game from a **high-risk lottery** into a **viable investment approach** with **favorable expected outcomes** and **manageable risk**.
...
Our modified strategy used a **50% bet size**, which is **five times larger** than the **Kelly-optimal 10%**. This explains our findings:

**Why the modified strategy performed well but not optimally:**

- At **50% bet size**, we achieved much better results than the original **100% bet**
...
- **Modified strategy (50% bet)** → **Substantial improvement** but still **suboptimal**
...
- Our **50% bet size** reduced but didn't eliminate the risk of **catastrophic losses**

**Practical Implications**

The **Kelly Criterion** demonstrates that **position sizing is crucial** in volatile investments. Even with a **positive expected value game**, **improper bet sizing** can turn a **winning strategy into a losing one**. Our analysis shows that moving from **100% to 50% betting** dramatically improved outcomes, but the **true optimal lies at 10%** according to Kelly.
